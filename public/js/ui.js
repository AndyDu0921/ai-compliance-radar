...
export class UIManager {
  ...
  showToast(message, type = 'info') {
    ...
    if (type === 'success') {
      ...
      iconSvg = '<svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>';
    }
  ...
  setStatus(message, state = "success") {
    ...
    if (this.dom.statusBadge) {
      ...
      if (state === "error") this.dom.statusBadge.classList.add("error");
      if (state === "processing") this.dom.statusBadge.classList.add("processing");
    }
  }
  ...
  updateMode(mode) {
    ...
    this.dom.textInput.placeholder = mode === "contract_review"
        ? "粘贴合同、协议、补充条款或采购单中的关键内容..."
        : "粘贴广告文案、直播脚本、落地页文案或促销活动内容...";
  }

  switchPane(paneId) {
    this.dom.paneButtons.forEach(btn => {
  ...
  renderRecentJobs(jobs, activeJobId, onJobClick) {
    ...
    this.dom.recentJobs.innerHTML = jobs.map(job => {
